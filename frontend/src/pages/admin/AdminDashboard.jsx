...
function UsersTab() {
    ...
    const [form, setForm]         = useState({
        username: "", email: "", password: "", role: "student", phone_number: ""
    });
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setMsg({ text: "❌ " + (typeof detail === "object" ? JSON.stringify(detail) : detail || "Failed to create user."), type: "error" });
    ...
    const ROLE_BADGE = {
        ...
        work_supervisor: "badge-blue",
        academic_supervisor: "badge-green",
        admin: "badge-yellow",
    };
        ...
        <>
            ...
            <div className="card">
                ...
                {loading ? (
                    ...
                    <div className="empty-state"><div className="empty-icon-svg">{Icon.users}</div><p>No users yet.</p></div>
                ) : (
                    <div className="table-wrap">
                        <table>
                            <thead>
                                <tr><th>Username</th><th>Email</th><th>Role</th><th>Phone</th><th></th></tr>
                            </thead>
                            <tbody>
                                {users.map(u => (
                                    <tr key={u.id}>
                                        ...
                                        <td>
                                            <span className={`badge ${ROLE_BADGE[u.role] || "badge-purple"}`}>
                ...
                <Modal title="Create New User" onClose={() => setShowForm(false)}>
                    <div className="form-grid">
                        <div className="field">
                            ...
                            <input placeholder="e.g. john_doe" value={form.username}
                                onChange={e => setForm({ ...form, username: e.target.value })} />
                        </div>
                        <div className="field">
                            <label>EMAIL</label>
                            <input type="email" placeholder="john@uni.ac.ug" value={form.email}
                        ...
                        <div className="field">
                            ...
                            <input placeholder="+256 700 000000" value={form.phone_number}
...
function PlacementsTab() {
    ...
    const [saving, setSaving]         = useState(false);
    const [form, setForm]             = useState({
        student: "", company_name: "", supervisor_name: "", startdate: "", enddate: ""
    });
    ...
    const handleDelete = async (p) => {
        ...
        try {
            ...
            setMsg({ text: "❌ Failed to delete placement.", type: "error" });
        }
    };
        ...
        <>
            ...
            <div className="card">
                ...
                {loading ? (
                    ...
                    <div className="empty-state"><div className="empty-icon-svg">{Icon.placement}</div><p>No placements yet. Assign students to companies.</p></div>
                ) : (
                    <div className="table-wrap">
                        <table>
                            <thead>
                                <tr><th>Student</th><th>Company</th><th>Work Supervisor</th><th>Start</th><th>End</th><th></th></tr>
                            </thead>
                            <tbody>
                                {placements.map(p => (
                                    <tr key={p.id}>
                                        ...
                                        <td>
                                            <button className="btn btn-danger btn-sm" onClick={() => handleDelete(p)}>{Icon.trash}</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                <Modal title="Assign Placement" onClose={() => setShowForm(false)}>
                    ...
                    <div className="form-grid">
                        <div className="field">
                            ...
                            <input placeholder="e.g. MTN Uganda" value={form.company_name}
                                onChange={e => setForm({ ...form, company_name: e.target.value })} />
                        </div>
                        <div className="field">
                            <label>WORKPLACE SUPERVISOR NAME</label>
                            <input placeholder="e.g. Alice Nakato" value={form.supervisor_name}
...
// ── ASSIGNMENTS TAB ───────────────────────────────────────────
// Assign work/academic supervisors to students
function AssignmentsTab() {
    ...
    const [showForm, setShowForm] = useState(false);
    const [saving, setSaving]     = useState(false);
    const [form, setForm]         = useState({ student: "", supervisor: "", type: "work" });
    ...
    const acadSupervisors   = users.filter(u => u.role === "academic_supervisor");
    const supervisorList    = form.type === "work" ? workSupervisors : acadSupervisors;

    const handleAssign = async () => {
        if (!form.student || !form.supervisor) {
            setMsg({ text: "Please select both a student and a supervisor.", type: "error" }); return;
        }
        setSaving(true); setMsg({ text: "", type: "" });
        try {
            // Update the placement record to link the supervisor
            // This patches the placement's supervisor field
            const field = form.type === "work" ? "work_supervisor" : "academic_supervisor";
            await api.patch(`/users/${form.student}/assign_supervisor/`, {
                [field]: form.supervisor,
            });
            setMsg({ text: "✅ Supervisor assigned successfully.", type: "success" });
            setShowForm(false);
            setForm({ student: "", supervisor: "", type: "work" });
        } catch (err) {
            // Fallback message if endpoint doesn't exist yet
            const status = err.response?.status;
            if (status === 404) {
                setMsg({ text: "⚠ The assign_supervisor endpoint doesn't exist yet in your backend. See README for how to add it.", type: "error" });
            } else {
                setMsg({ text: "❌ Assignment failed. " + (err.response?.data?.detail || ""), type: "error" });
            }
        } finally { setSaving(false); }
    };
    ...
    return (
        <>
            ...
            <div className="card">
                ...
                {loading ? (
                    ...
                    <div className="table-wrap">
                        <table>
                            <thead>
                                <tr><th>Student</th><th>Email</th><th>Role</th></tr>
                            </thead>
                            <tbody>
                                {students.length === 0 ? (
                                    <tr><td colSpan={3} style={{ textAlign: "center", color: "var(--muted)", padding: 32 }}>No students found.</td></tr>
                                ) : students.map(s => (
                                    <tr key={s.id}>
                                        <td><strong>{s.username}</strong></td>
                                        <td className="text-muted text-sm">{s.email}</td>
                                        <td><span className="badge badge-purple">Student</span></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            {showForm && (
                <Modal title="Assign Supervisor to Student" onClose={() => setShowForm(false)}>
                    <div className="field">
                        <label>SUPERVISOR TYPE</label>
                        <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value, supervisor: "" })}>
                            <option value="work">Workplace Supervisor</option>
                    ...
                    <div className="field">
                        <label>{form.type === "work" ? "WORKPLACE" : "ACADEMIC"} SUPERVISOR</label>
                        <select value={form.supervisor} onChange={e => setForm({ ...form, supervisor: e.target.value })}>
                            <option value="">— Select supervisor —</option>
                            {supervisorList.length === 0
                                ? <option disabled>No {form.type === "work" ? "work" : "academic"} supervisors found</option>
                                : supervisorList.map(s => <option key={s.id} value={s.id}>{s.username} ({s.email})</option>)
                            }
                        </select>
                    </div>
                    {msg.text && <div className={`alert ${msg.type === "error" ? "alert-error" : "alert-success"}`}>{msg.text}</div>}
            ...
            <div className="admin-body">
                ...
                <div className="stats-grid" style={{ marginBottom: 28 }}>
                    <div className="stat-card">
                        ...
                        <div className="stat-value stat-accent">{stats.users}</div>
                    </div>
                    <div className="stat-card">
                        <div className="stat-label">Placements</div>
                        <div className="stat-value stat-green">{stats.placements}</div>
                    </div>
                    <div className="stat-card">
                        <div className="stat-label">Weekly Logs</div>
                        <div className="stat-value stat-yellow">{stats.logs}</div>
                    </div>
                </div>
